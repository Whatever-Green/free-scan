...
_.extend(a5.dp.config, {
  ...
  bookNotesExportFormats: function() {                                    // comma separated list with the allowed export formats for the book notes
  ...
  bookZoomPinchStep: function() {                                         // Default % zoom step for digital books on pinch gesture
  ...
  bookHotspotOpenInTab: function() {                                      // Allows to override dialog/tab setting for external and platform hotspots. No override if null. May be a function that gets the setting value as a string (default: null, options: 'tab' | 'iframe')
    if (window.outerWidth <= 988) {
      return 'tab';
    } else {
      return null;
    }
